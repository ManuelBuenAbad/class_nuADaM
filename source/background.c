...
int background_indices(
 ...
 /** START #TWIN SECTOR */
  ...
  //MBA
  if ((pba->background_verbose > 1) && (pba->has_twin)) {
    printf(".............................................\n");
    printf("TWIN-BA: checking whether there is IDM-DR/IDR when defining indices:\n");
  ...
  if ((pba->background_verbose > 1) && (pba->has_twin)) {
    printf(".............................................\n");
    printf("TWIN-BA: checking the pointer/index redundancy:\n");
    printf("\tpba->index_bg_rho_idr = %i\n\tpba->index_bg_rho_g_twin = %i\n\tpba->index_bg_rho_ur_twin = %i\n\tpba->index_bg_rho_idm_dr = %i\n\tpba->index_bg_rho_b_twin = %i\n", pba->index_bg_rho_idr, pba->index_bg_rho_g_twin, pba->index_bg_rho_ur_twin, pba->index_bg_rho_idm_dr, pba->index_bg_rho_b_twin);
    printf(".............................................\n");
  }
  //MBA
...
int background_checks(
  ...
  if (pba->r_all_twin != 0.) {
    ...
    if (pba->background_verbose > 0) {
      printf(".............................................\n");
      printf("TWIN-BA: checking twin components\n");
      ...
      //MBA
      printf(" -> r_all_twin = %g\n",pba->r_all_twin);
      printf(" -> Omega0_b_twin/Omega0_cdm = %g\n",pba->Omega0_b_twin/pba->Omega0_cdm);
      printf(" -> Omega0_b_twin/(Omega0_cdm + Omega0_b_twin) = %g\n",pba->Omega0_b_twin/(pba->Omega0_cdm + pba->Omega0_b_twin));
      printf(" -> Omega0_cdm + Omega0_b_twin = %g\n",pba->Omega0_b_twin + pba->Omega0_cdm);
      printf(" -> Omega0_g + Omega0_ur + Omega0_b + Omega0_cdm + Omega0_g_twin + Omega0_b_twin + Omega0_lambda = %g\n",pba->Omega0_g + pba->Omega0_ur + pba->Omega0_b + pba->Omega0_cdm + pba->Omega0_g_twin + pba->Omega0_b_twin + pba->Omega0_lambda);
      printf(".............................................\n");
      //MBA
    }
  }
...
int background_output_titles(
  ...
  if ((pba->background_verbose > 1) && (pba->has_twin)) {
    printf(".............................................\n");
    printf("TWIN-BA: checking whether there is IDM-DR/IDR when storing column titles (in output):\n");
    printf("\tpba->has_idr = %i\n", pba->has_idr);
    printf("\tpba->has_idm_dr = %i\n", pba->has_idm_dr);
    printf(".............................................\n");
  }
  ...
  // class_store_columntitle(titles,"(.)rho_idr",pba->has_twin);//MBA
  // class_store_columntitle(titles,"(.)rho_idm_dr",pba->has_twin);//MBA
  /** END TWIN SECTOR */
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    // class_store_double(dataptr,pvecback[pba->index_bg_rho_idr],pba->has_twin,storeidx);//MBA
    // class_store_double(dataptr,pvecback[pba->index_bg_rho_idm_dr],pba->has_twin,storeidx);//MBA
    /** END TWIN SECTOR */
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    /* TWIN - index for rho_b_twin (mirror baryon density) */
    if(pba->has_twin){
      class_print_species("Twin Baryon Dark Matter",b_twin);
      budget_matter+=pba->Omega0_b_twin;//MBA
    }
    /** END TWIN SECTOR */
    ...
    if(pba->has_twin){
      ...
      // budget_matter+=pba->Omega0_b_twin;//MBA: moved it up
    }
    /** END TWIN SECTOR */
    if (pba->has_dr == _TRUE_) {
