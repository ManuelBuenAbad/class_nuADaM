        ...
        if ((pa_old[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_on) && (ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off)) {
          ...
          if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_idr_off) {
            ...
            if (ppt->idr_nature == idr_free_streaming){
              ...
              //nuADM
              ppv->y[ppv->index_pt_l3_idr] = 6./7.*k*ppv->y[ppv->index_pt_shear_idr]/(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.))/ppt->alpha_idm_dr[1];
              //nuADM
            }
          }
    ...
    if(pba->has_idm_dr == _TRUE_){
      if(ppw->pvecthermo[pth->index_th_dmu_idm_dr] == 0.){
        ...
        class_test(1./ppw->pvecthermo[pth->index_th_dmu_idm_dr] < 0.,
                   ...
                   //nuADM
                   1./(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.)),
                   //nuADM
                   1./ppw->pvecback[pba->index_bg_a]-1.,
                   tau);

        //chnu:MBA: what is this? why are the triggers set to 0?
        if(pba->has_twin == _TRUE_){
        ...
        //chnu:MBA
        //nuADM
        if ((1./tau_h/(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.)) < ppr->idm_dr_tight_coupling_trigger_tau_c_over_tau_h) &&
            (1./tau_k/(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.)) < ppr->idm_dr_tight_coupling_trigger_tau_c_over_tau_k) &&
    ...
    if (pba->has_idr == _TRUE_){
      ...
      if(pba->has_idm_dr==_TRUE_){
        ...
        if ((tau/tau_k > ppr->idr_streaming_trigger_tau_over_tau_k) &&
            ...
            //mba_idr_rsa
            ((tau/pba->tau_eq > ppr->idr_streaming_trigger_tau_over_tau_eq)) &&
            //mba_idr_rsa
            (pth->nindex_idm_dr>=2)) &&
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if ((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_on)){
        //nuADM
        shear_idr = 0.5*8./15./(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.))/ppt->alpha_idm_dr[0]*(y[ppw->pv->index_pt_theta_idr]+k2*ppw->pvecmetric[ppw->index_mt_alpha]);
      ...
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {
        ...
        if (ppt->idr_nature == idr_free_streaming){
          if((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_on)){
            if(ppt->gauge == newtonian)
              //nuADM
              shear_idr = 0.5*(8./15./(ppw->pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.))/ppt->alpha_idm_dr[0]*(y[ppw->pv->index_pt_theta_idr]));
            else
...
int perturbations_derivs(double tau,
  ...
  if((pba->has_idm_dr==_TRUE_)){
    ...
    //nuADM: rescale by extra d.o.fs
    dmu_idm_dr = pvecthermo[pth->index_th_dmu_idm_dr]/(1.+pba->dof_extra_twin/2.);
    //nuADM
    ...
    if (ppt->perturbations_verbose > 3) {
      ...
      if (random_num > 990) {
        printf("TWIN-PT: computing ETHOS DR-DR and DR-IDM interaction coeffients and rates\n");
        printf("\tpth->b_idr = %g, pth->a_idm_dr = %g\ndmu_idm_dr = %g, dmu_idr = %g, dmu_idr*beta_idr(=0) = %g\n", pth->b_idr, pth->a_idm_dr, dmu_idm_dr, dmu_idr, dmu_idr*0.);
      }
    }
    //MBA
    //chnu
