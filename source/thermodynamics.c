...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if (pba->has_idm_dr == _TRUE_) {
      ...
      //MBA
      if ((pth->thermodynamics_verbose > 3) && (pba->has_twin)) {
        printf(".............................................\n");
        printf("TWIN-TH: checking whether there is IDM-DR/IDR at high-z:\n\t[!!!] WARNING: will print many times because this is called many times [!!!]\n");
        printf("\tpba->has_idm_dr = %i\n", pba->has_idm_dr);
        printf("\tpba->has_idr = %i\n", pba->has_idr);
        printf(".............................................\n");
      }
      //MBA
...
int thermodynamics_init(
  ...
  /* BEGIN #TWIN SECTOR */
  if (pba->has_twin == _TRUE_) {
  pth->fHe_twin = pth->YHe_twin/(_not4_ *(1.-pth->YHe_twin));//MBA: assumes the twin Helium-to-Hydrogen ratio is the same as in the SM
  }
  /* END TWIN SECTOR */
...
int thermodynamics_helium_from_bbn(
  ...
  //nuADAM
  // ignore DeltaNeff during BBN
  if ((pba->has_twin == _TRUE_) && (pba->ignore_DNtwin_BBN == _TRUE_)) {
...
int thermodynamics_vector_init_twin(
  ...
  /* - in this scheme we start to evolve Helium and thus need to set its initial condition via the analytic function */
  /**else if (ptdw->ap_current_twin == ptdw->index_ap_H_twin) {hyrec_dx_H_dz
    /* Store Tmat in workspace for later use
...
int thermodynamics_derivs_twin(
  ...
  x_twin = ptdw->x_noreio_twin;
    ...
    if (ptdw->require_H_twin == _TRUE_) {
        ...
        class_call(hyrec_dx_H_dz_chnu(pth,ptw->ptdw->phyrec_twin,x_H_twin,0.,x_twin,nH_twin,z,Hz,Tmat_twin,Trad_twin,pba->alphafs_dark/0.00729735,_mu_twin_/((_m_e_*_m_p_)/(_m_e_+_m_p_)),&(dy[ptv->index_ti_x_H_twin])),
                ptw->ptdw->phyrec_twin->error_message,
                error_message);
...
//Load Fp-r array, ToB, TMoTR values.
  ...
  //MBA: note the denominator of the definition of epsilon here (Trad_twin, which doesn't match that used in 2212.02487 but matches that of 1209.5752), and also the comments
  ...
  **/
  ...
  Gamma_rayleigh = 2*(2*_PI_*pow(_k_B_,9)/_h_P_/pow(_c_,6))*430080. * 1.002008 * pow(pba->alphafs_dark,2) * nH_twin * (1-x_twin) * pow(Trad_twin,8)/(pow(_PI_,2) * pow(pow(pba->alphafs_dark,2)*_mu_twin_*_c_*_c_/2.,4)*(_m_H_twin)*pow(_m_e_twin,2))/(3 * _k_B_ * nH_twin*(1.+x_twin+ptw->fHe_twin));
   ...
   dy[ptdw->ptv->index_ti_D_Tmat_twin] = myrate;
    ...
    //MBA: note the sign difference of the Rayleigh term here (which differs from that in 2212.02487 and 1209.5752), and also the comments
...
int thermodynamics_sources_twin(
  ...
  //chnu: what is this at all?: MBA: did you comment this out? //TY: yes, it print a funny Tmat
  ...
  if (Trad_twin/ptw->const_Tion_H_twin < 0.1){
    ...
    //pth->thermodynamics_table[(pth->tt_size-index_z-1)*pth->th_size+pth->index_th_dkappa_twin] = (1.+z) * (1.+z) * ptw->SIunit_nH0_twin * x_twin * _sigma_twin * (1. + pow(pba->m_e_dark/pba->m_p_dark,2)) * _Mpc_over_m_ + 32 * pow(_PI_,4) * (1.+z) * (1.+z) * ptw->SIunit_nH0_twin * (1-x_twin) * _sigma_twin * _Mpc_over_m_ * pow(Trad_twin/ptw->const_Tion_H_twin,4);
    //nuADM: add p-i scattering
    pth->thermodynamics_table[(pth->tt_size-index_z-1)*pth->th_size+pth->index_th_dkappa_twin] = (1.+z) * (1.+z) * ptw->SIunit_nH0_twin * x_twin * _sigma_twin * (1. + pow(pba->m_e_dark/pba->m_p_dark,2)) * _Mpc_over_m_ + 32 * pow(_PI_,4) * (1.+z) * (1.+z) * ptw->SIunit_nH0_twin * (1-x_twin) * _sigma_twin * _Mpc_over_m_ * pow(Trad_twin/ptw->const_Tion_H_twin,4)+ (1.+z)*(1.+z) * ptw->SIunit_nH0_twin * (1-x_twin) * sqrt(_PI_/2.) * exp(-ptw->const_Tion_H_twin/(Trad_twin)) * sqrt(_m_e_) * _m_e_ * pow(pba->alphafs_dark/0.00729735,3) * alphaA_PPB(Trad_twin, 1., 1.) / (4. * 1.2020569 * Trad_twin * sqrt(Trad_twin)) * pow(10,7.5)/(pow(_GeV_over_kg_,1.5)*(_s_over_Mpc_)*pow(_eV_over_Kelvin_,1.5));
    //nuADM
    //chnu
  }
  else{
...
int thermodynamics_output_titles(
  ...
  /** START #TWIN SECTOR */
  //MBA
  if ((pth->thermodynamics_verbose > 1) && (pba->has_twin)) {
    printf(".............................................\n");
    printf("TWIN-TH: checking whether there is IDM-DR/IDR when storing column titles (in output):\n");
    printf("\tpba->has_idr = %i\n", pba->has_idr);
    printf("\tpba->has_idm_dr = %i\n", pba->has_idm_dr);
    printf(".............................................\n");
  }
  ...
  if(pba->has_twin == _TRUE_){
    ...
    // class_store_columntitle(titles,"tau_idr",_TRUE_);//MBA
    // class_store_columntitle(titles,"dmu_idr",_TRUE_);//MBA
  }
  /** END TWIN SECTOR */
  if (pba->has_idm_dr == _TRUE_) {
...
int thermodynamics_output_data(
  ...
  for (index_z=0; index_z<pth->tt_size; index_z++) {
    ...
    if(pba->has_twin == _TRUE_){
      ...
      // class_store_double(dataptr,pvecthermo[pth->index_th_tau_idr],_TRUE_,storeidx);//MBA
      // class_store_double(dataptr,pvecthermo[pth->index_th_dmu_idr],_TRUE_,storeidx);//MBA
    }
    /** END TWIN SECTOR */
