...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_background){
    ...
    ba.background_verbose = 1;//MBA_temp
  ...
  if (pfzw->required_computation_stage >= cs_thermodynamics){
   if (input_verbose>2)
    ...
    pr.thermo_Nz_lin = 10000;//MBA: was 10000
    pr.thermo_Nz_log = 500;//MBA: was 500
//TWIN
//    pr.thermo_Nz_log_if_twin = 10000;//MBA: new line
//TWIN
    th.thermodynamics_verbose = 1;//MBA_temp
    th.hyrec_verbose = 0;
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  //chnu: read extra degree of freedom
  class_call(parser_read_double(pfc,"dof_extra_twin",&param1,&flag1,errmsg),
              errmsg,
              errmsg);
      if (flag1 == _TRUE_) {
        pba->dof_extra_twin = param1;
      }
  //chnu

  //nuADAM
  class_call(parser_read_string(pfc,"ignore Delta_N_twin at BBN",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_){
    if (string_begins_with(string1,'y') || string_begins_with(string1,'Y'))
      pba->ignore_DNtwin_BBN = _TRUE_;
    else
      pba->ignore_DNtwin_BBN = _FALSE_;
  }
  //nuADAM
  ...
  class_test(class_at_least_two_of_three(flag1,flag2,flag3),
      ...
      if (flag1 == _TRUE_) {
        ...
        pba->xi_twin = pow((7./8.)*pow(4./11.,4./3.) * pba->Delta_N_twin / (1.+pba->dof_extra_twin/2.),1./4.);
        //chnu
      }
      else if (flag2 == _TRUE_) {
        ...
        pba->Delta_N_twin = (1.+pba->dof_extra_twin/2.) * (8/7.)*pow(11./4.,4./3.) * pow(pba->xi_twin,4.);
        //chnu
      }
     /* Temp FLAG1 */
...
*/
    ...
    //nuADAM: omega_dm_tot to fix z_eq
  ...
  /* Complete set of parameters */
  if (flag1 == _TRUE_){
    pba->Omega0_cdm =  (1. - pba->r_all_twin) * param1;
    has_cdm_userdefined = _TRUE_;
  }
  if (flag2 == _TRUE_){
    pba->Omega0_cdm = (1. - pba->r_all_twin) * param2/pba->h/pba->h;
    has_cdm_userdefined = _TRUE_;
  }
  //nuADAM
    ...
    if(pba->r_all_twin!=0.){
      ...
      // pba->T0_twin = pba->T_cmb*pow(pba->Delta_N_twin/4.4/(1.+pba->dof_extra_twin/2.),1./4.);//MBA
      pba->T0_twin = pba->T_cmb*pow((pba->Delta_N_twin*(7./8.)*pow(4./11.,4./3.))/(1.+pba->dof_extra_twin/2.),1./4.);//MBA
      ...
      pba->Omega0_g_twin = (1.+pba->dof_extra_twin/2.) * pow(pba->T0_twin/pba->T_cmb,4.)*pba->Omega0_g;
      //chnu
      pba->Omega0_ur_twin = 0; /* Old, general adm doesn't have neutrinos: *7./8.*3*pow(pba->T0_ur_twin/pba->T_cmb,4.)*pba->Omega0_g */
      // pba->Omega0_b_twin = pba->r_all_twin*pba->Omega0_cdm-pba->Omega0_g_twin-pba->Omega0_ur_twin;//MBA: seems very wrong
      pba->Omega0_b_twin = (pba->r_all_twin / (1. - pba->r_all_twin))*pba->Omega0_cdm;//MBA
      ...
      // pba->Omega0_cdm -= pba->Omega0_b_twin;//MBA: NO!!!! This will mess up the MCMC chains!!!!
      ...
      if (input_verbose > 1) {
        printf(".............................................\n");
        printf("TWIN-IN:\n\tmade Omega0_idr = Omega0_g_twin and T_idr = T0_twin\n");
        printf(".............................................\n");
      }
      //MBA
  ...
  if (flag3 == _TRUE_) {
    ...
    if(pba->r_all_twin!=0.){
      ...
      if (input_verbose > 1) {
        printf(".............................................\n");
        printf("TWIN-IN:\n\tadded Omega0_idm_dr to Omega0_cdm, for some reason that I don't think can be right\n");
        printf(".............................................\n");
      }
      //MBA
    };
  ...
  if(pba->r_all_twin!=0.){
    ...
    if (input_verbose > 1) {
      printf(".............................................\n");
      printf("TWIN-IN:\n\tmade Omega0_idm_dr = Omega0_b_twin\n");
      printf(".............................................\n");
    }
    //MBA
