...
//chnu
int hyrec_dx_H_dz_chnu(struct thermodynamics* pth, struct thermohyrec* phy, double x_H, double x_He, double xe, double nH, double z, double Hz, double Tmat, double Trad, double alphaR, double meR, double *dx_H_dz) {
  /* For twin sector: meR should really be reduced mass ratio mu_D/mu_SM */
  /** Summary: */
  ...
  /** - convert to correct units, and retrieve derivative */
  *dx_H_dz = -1./(1.+z)* rec_dxHIIdlna_chnu(phy->data, model, xe, x_H, nH*1e-6, Hz, Tmat*kBoltz, Trad*kBoltz, iz, z);

  /** - do error management */
  if(phy->data->error != 0){
    class_call_message(phy->error_message,"rec_dxHIIdlna_chnu",phy->data->error_message);
    return _FAILURE_;
  }
