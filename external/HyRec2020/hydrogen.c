...
//chnu
/**************************************************************************************************
Case-A recombination coefficient and photoionization rate, fit of Pequignot et al 1991, in cm^3 s^{-1}
INPUT TEMPERATURE ASSUMED TO BE ALREADY RESCALED FOR VALUES OF alpha_fs and me
***************************************************************************************************/

double alphaA_PPB(double TM, double fsR, double meR) {
  double t4;

  t4 = TM/kBoltz/1e4;
  return square(fsR/meR) * 5.596e-13*pow(t4,-0.6038)/(1.+ 0.3436*pow(t4,0.4479));
}
//chnu
...
//chhu
double rec_TLA_dxHIIdlna_chnu(REC_COSMOPARAMS *cosmo, double xe, double xHII, double nH, double H, double TM, double TR, double Fudge) {
  
  double fsR = cosmo->fsR, meR = cosmo->meR;
  double alphaA_TM, alphaA_TR, s, Dxe2, DalphaA;

  rescale_T(&TM, fsR, meR);
  rescale_T(&TR, fsR, meR);
  //Need to regulate divergence when xHII = 1.

  alphaA_TM  = Fudge * alphaA_PPB(TM, fsR, meR);
  alphaA_TR  = Fudge * alphaA_PPB(TR, fsR, meR);
  s          = SAHA_FACT(fsR, meR) *TR*sqrt(TR) *exp(-EI/TR)/nH;
  Dxe2       = xe*xHII - s*(1.-xHII);    /* xe xp - xe xp[Saha eq with 1s] -- gives more compact expressions */
  DalphaA    = alphaA_TM - alphaA_TR;

  return  -nH*(s*(1.-xHII)*DalphaA + Dxe2*alphaA_TM)/H;
...
double rec_dxHIIdlna_chnu(HYREC_DATA *data, int model, double xe, double xHII, double nH, double H, double TM, double TR,
                     unsigned iz, double z){
  ...
  result = rec_TLA_dxHIIdlna_chnu(cosmo, xe, xHII, nH, H, TM, TR, 1.00);
