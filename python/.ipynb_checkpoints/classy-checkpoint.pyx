...
cdef class Class:
    ...
    # MANUEL: below has been changed cpdef -> cdef
    ...
    # MANUEL: above has been changed cpdef -> cdef
    ...
    def tau_eq(self):
        return self.ba.tau_eq

    def Neff_bbn(self):
        return self.ba.Neff_bbn

    def r_all_twin(self):
        return self.ba.r_all_twin
    ...
    #MANUEL
    def Dn_pivot(self, kbounds, kstar, zstar, model='power_law', Npts=201):
        return Dn_fit(self, kbounds, kstar, zstar, model, Npts)
    
    def Dn_pivot_raw(self, kbounds, kstar, zstar, Npts=201):
        return Dn_raw(self, kbounds, kstar, zstar, Npts)
    #MANUEL
    ...
    def get_current_derived_parameters(self, names):
        ...
        #MANUEL: ADAM
        if (('Delta_k009_z3' in names) or ('neff_k009_z3' in names)):
            if self.ba.r_all_twin == 0:
                Dn_values = self.Dn_pivot([0.001, 0.08], 0.009, 3, model='power_law_log')
            else:
                Dn_values = self.Dn_pivot([0.001, 0.08], 0.009, 3, model='power_law_log_sup')
            
        if (('Delta_k009_z3_raw' in names) or ('neff_k009_z3_raw' in names)):
            Dn_values_raw = self.Dn_pivot_raw([0.001, 0.08], 0.009, 3)
        #MANUEL: ADAM
        ...
        for name in names:
            ...
            #MANUEL: ADAM
            elif name == 'Delta_k009_z3':
                value = Dn_values[0]
            elif name == 'neff_k009_z3':
                value = Dn_values[1]
            elif name == 'Delta_k009_z3_raw':
                value = Dn_values_raw[0]
            elif name == 'neff_k009_z3_raw':
                value = Dn_values_raw[1]
            #MANUEL: ADAM
            #END TWIN SECTOR
            elif name == 'Omega_m':
...
def find_pk_fit(self, kbounds, kstar, zstar, model='power_law', Npts=201):
    ...
    if model == 'power_law_log':
        ...
        pars2, _ = curve_fit(to_fit, np.log10(k_arr), new_yarr, ref_vals, bounds=(lower_bounds, upper_bounds), maxfev=50000)
